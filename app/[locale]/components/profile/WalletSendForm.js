...
import { checkIfEmailExist, getUserData, sendMoney } from "@/app/api/supabase/user";
...
export const WalletSendForm = ({ setOpenForm, setRefreshWallet }) => {
  ...
  return (
    <form onSubmit={onSubmitSend} className="flex flex-col gap-2">
      ...
      <div className="flex gap-2 items-center mt-4">
        <button
          disabled={isLoading}
          className="w-full max-w-[250px] disabled:bg-gray-300 disabled:text-gray-700 hover:shadow-md p-2 rounded-md text-sm bg-opink text-white "
        >
          {t("send")}
        </button>
        <button
          disabled={isLoading}
          onClick={() => setOpenForm("")}
          className="border border-red-500 text-red-500 disabled:border-0 disabled:bg-gray-300 disabled:text-gray-700 hover:text-white hover:bg-red-500 p-2 text-sm rounded-md capitalize"
        >
          {t("cancel")}
        </button>
      </div>
