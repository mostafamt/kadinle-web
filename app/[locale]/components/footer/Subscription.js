...
export const Subscription = ({ locale }) => {
  ...
  const handleSubscribe = async () => {
    ...
    subscribe(email).then((res) => {
      if (!res?.error) {
        ...
        checkIfEmailHasSubscribe().then((res) => {
          if (!res) increasePointsByType("SUBSCRIPTION", user?.id);
        });
      } else {
        setMsg("warning");
      }
  ...
  return (
    <div className="relative h-[184px] pt-6 overflow-hidden">
      ...
      <h3 className="relative mx-4 hidden md:block max-w-fit md:mx-auto p-2 rounded-md bg-gray-50 mb-6 !text-sm text-opink z-10 whitespace-normal">
        {t("newsletter_msg")}
      </h3>
      <div className="flex items-center md:items-start">
        <div className="z-10 block bg-dblue py-2 xl:py-4 ltr:rounded-r-3xl rtl:rounded-l-3xl px-5">
