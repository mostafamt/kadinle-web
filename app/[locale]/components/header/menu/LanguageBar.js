...
export const LanguageBar = ({ languages, locale }) => {
  ...
  return (
    <div className="relative">
      <button
        className={`text-[#727C8E] text-[14px] capitalize flex items-center gap-1 border p-1 rounded-md ${
      ...
      {open ? (
        <>
          ...
          />
          <div className="absolute bg-white shadow rounded-md z-[10001] top-full ltr:right-0 rtl:left-0">
            {languages?.map((lang) => (
              <button
                key={lang?.id}
                onClick={() => {
