...
const NewsBar = ({ setOpenNews, news, locale }) => {
  ...
  return (
    <div className=" bg-yellow-200 text-black direction-ltr">
      <div className="flex container items-center justify-between gap-2 p-1 direction-ltr">
        <Marquee
          speed={50}
          pauseOnHover={true}
        ...
        <button
          ...
          className=" flex items-center justify-center text-black rounded-full w-7 h-7"
        >
          <CloseIcon className="w-5 h-5 text-b" />
        </button>
      </div>
    </div>
  );
