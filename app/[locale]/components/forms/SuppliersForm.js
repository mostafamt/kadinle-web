...
export const SuppliersForm = () => {
  ...
  const onSubmit = async (e) => {
    ...
    fetch(`https://kadinle.com/api/suppliers`, {
      ...
      .then((res) => res.json())
      .then(async (data) => {
        const response = await addNewSupplierRegistration({
          ...formInformation,
          attachment: data?.url,
          user_id: user?.id,
        });
        if (!response?.error) {
          toast.success(t("supplier_success"));
          await addNewSupplier({
            company_name: formInformation?.company_name,
            attachment: data?.url,
            user_id: user?.id,
            supplier_request_id: response?.data?.at(0)?.id,
            supplier_number_id: new ShortUniqueId({
              dictionary: "number",
              length: 8,
            })?.dict?.join(""),
          });
  ...
  return (
    <>
      ...
      <div className="">
        ...
        {user?.id ? null : (
          <p className="text-red-500 text-center flex flex-col gap-2 items-center justify-center">
            {t("required_login")}
            <button
              className="bg-primary py-1 px-4 rounded-md mx-2 text-white capitalize font-medium w-full max-w-[120px]"
        ...
        <form onSubmit={onSubmit} className="px-4 mt-2">
          ...
          <div className="mb-4 text-sm flex flex-col gap-4">
            <SelectField
              containerClassName="!mb-4"
          ...
          <button
            disabled={!user?.id}
            className="bg-opink text-white disabled:bg-gray-200 disabled:text-gray-400 p-2 rounded-md block mx-auto my-4 px-8"
          >
            {t("submit")}
          </button>
        </form>
